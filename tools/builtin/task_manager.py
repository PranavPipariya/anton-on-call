...
from tools.tool_interface import Tool, ToolInvocation, ToolKind, ToolResult
from pydantic import BaseModel, Field


class TodosParams(BaseModel):
    action: str = Field(..., description="Action: 'add', 'complete', 'list', 'clear'")
    id: Optional[str] = Field(None, description="Todo ID (for complete)")
    content: Optional[str] = Field(None, description="Todo content (for add)")
